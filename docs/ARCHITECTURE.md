...
- `core/storage.py`：规则、草稿、命中与冷却状态，以及本地消息关键词索引。
...
```text
命令立即确认并转入后台任务（规避宿主命令超时）
  → 模型提取查询目标、关键词、排除词和时间（用户原文始终参与检索）
  → 解析目标群真实聊天流
  → 关键词直查本地索引；索引未覆盖的更早时段分页扫描补齐并写入索引
  → 英文词按独立词过滤误命中，先评分再截断
  → 可选 ctx.llm.embed 语义重排
  → 按时间片段分散选取证据种子，从索引补齐前后对话
  → 可选外部查询
  → 事实复核模型根据 E/S 编号回答，清洗无效引用后附证据原文和检索说明
  → 结果通过消息发送到发起会话
```

本地索引由 ON_MESSAGE 旁路观察实时写入（只限白名单群），寻迹的补扫描也会回填索引。
每次查询都会让索引往回加深一层，越用覆盖越完整。索引关闭时回退为纯分页扫描。
